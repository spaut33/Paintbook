...
class PaintsApiTestCase(APITestCase):
    def setUp(self):
        ...
        series = Series.objects.create(
            name='Test Series 1', manufacturer=manufacturer
        )
    ...
    def test_get_list(self):
        ...
        paints = Paint.objects.all().annotate(
    ...
    def test_get_filter(self):
        url = reverse('paint-list')
        paints = (
            Paint.objects.filter(id=self.paint_2.id)
            .annotate(
                average_rating=Avg('userpaint__rating'),
                max_rating=Max('userpaint__rating'),
            )
    ...
    def test_get_search(self):
        ...
        paints = Paint.objects.filter(
            id__in=[self.paint_1.id, self.paint_3.id]
        ).annotate(
            average_rating=Avg('userpaint__rating'),
            max_rating=Max('userpaint__rating'),
        )
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            PaintSerializer(paints, many=True).data, response.data
        )
    ...
    def test_update_not_owner(self):
        ...
        # Test returns 403 error, and it's ok because not owner shouldn't
        # change the paint
    ...
    def test_delete_not_owner_but_staff(self):
        # Test for deleting the paint if user is not the owner but has
        # is_staff flag
        url = reverse('paint-detail', args=(self.paint_3.id,))
        self.assertEqual(3, Paint.objects.all().count())
